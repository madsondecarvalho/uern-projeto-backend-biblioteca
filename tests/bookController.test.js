...
describe('bookController', () => {
  ...
  it('returns all books', () => {
    const res = response();
    const books = [{ id: 1 }];
    jest.spyOn(bookService, 'listBooks').mockReturnValue(books);

    getAllBooks(null, res);

    expect(res.json).toHaveBeenCalledWith(books);
  });

  it('returns a book when service resolves', () => {
    const res = response();
    const req = { params: { id: '1' } };
    const book = { id: 1 };
    jest.spyOn(bookService, 'getBookById').mockReturnValue(book);
  ...
  it('creates a book and returns 201', () => {
    const body = { title: 'book', author: 'author', year: 2024, available: true };
    const req = { body };
    const res = response();
    jest.spyOn(bookService, 'createBook').mockReturnValue({ ...body, id: 1 });
  ...
  it('replaces a book', () => {
    ...
    jest.spyOn(bookService, 'replaceBook').mockReturnValue(updated);
  ...
  it('deletes a book and sends 204', () => {
    const req = { params: { id: '1' } };
    const res = response();
